...
Повторный запуск не открывает вторую копию, а разворачивает уже запущенное окно.
...
Модели хранятся в `%LOCALAPPDATA%\SubForStream\models`. Во вкладке **«Модели»** видно, что уже скачано
и сколько места занимает. Там же модель можно скачать заранее или удалить.
...
Порт занят другим приложением или старой версией SubForStream (до 1.1 защиты от второй копии не было).
...
- `single_instance.py` — защита от второй копии: повторный запуск показывает открытое окно.
